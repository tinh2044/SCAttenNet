...
class SLR_Dataset(Dataset.Dataset):
    def __init__(self, root, cfg, split, gloss_tokenizer=None):
        self.cfg = cfg
    ...
    def data_collator(self, batch):
        ...
        gloss_output = self.gloss_tokenizer.batch_encode(
            gloss_batch, return_length=True
        )
    ...
    def select_frames(self, keypoints):
        ...
        if self.split != "train":
            ...
            assert len(frames_idx) <= self.max_len, (
                f"len(frames_idx) = {len(frames_idx)} > {self.max_len}"
            )
