...
class RecognitionHead(nn.Module):
    def __init__(self, cfg, gloss_tokenizer):
        ...
        #     cfg["residual_blocks"][-1], len(gloss_tokenizer)
        # )
        self.fuse_gloss_classifier = nn.Linear(
            cfg["residual_blocks"][-1], len(gloss_tokenizer)
        )
    ...
    def forward(
        ...
        fuse_coord_logits = self.fuse_gloss_classifier(fuse_output)
        outputs = {
...
class MSCA_Net(torch.nn.Module):
    def __init__(self, cfg, gloss_tokenizer, device="cpu"):
        ...
        # )
        self.keypoint_encoder = SeparativeCoordinateAttention(
            cfg["fuse_idx"], num_frame=cfg["num_frame"], cfg=cfg
        )
    ...
    def forward(self, src_input, **kwargs):
        ...
        # )  # (B, 3D, T/4)
        keypoints = src_input["keypoints"][:, :, self.cfg["fuse_idx"], :]
        mask = src_input["mask"]
        fuse_embed = self.keypoint_encoder(keypoints, mask)
        head_outputs = self.recognition_head(fuse_embed)
        ...
        if self.self_distillation:
            ...
            student_logits = outputs["fuse_coord_logits"]
            ...
            outputs["fuse_distill_loss"] = self.distillation_loss(
                student_logits, teacher_logits, use_blank=False
            )
            if torch.isnan(outputs["fuse_distill_loss"]) or torch.isinf(
                outputs["fuse_distill_loss"]
            ):
                raise ValueError("NaN or inf in fuse_distill_loss")

            outputs["total_loss"] += outputs["fuse_distill_loss"]
